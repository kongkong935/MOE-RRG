    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in tqdm(enumerate(self.test_dataloader)):
                ...
                report        =   batch["report"].to(self.device)
                reports_mask  =   batch["report_mask"].to(self.device)
                ...
                moe_inputs = {"view_ids": viewposition_MOE, "visit_ids": visitorder_MOE, "aux_text": aux_text,
                              "impression": imp, "imp_mask": imp_mask}

                output, _,top1 = self.model(image_processor,moe_inputs, mode='sample')
                pred_txt  = self.model.tokenizer.decode_batch(output.cpu().numpy())
                gt_txt    = self.model.tokenizer.decode_batch(report[:, 1:].cpu().numpy())
                # ---------- 收集到 test_rows ----------
                for i in range(len(pred_txt)):  # len(pred_txt) == batch_size
                    test_rows.append({
                        "view_id": int(viewposition_MOE[i].item()),
                        "visit_id": int(visitorder_MOE[i].item()),
