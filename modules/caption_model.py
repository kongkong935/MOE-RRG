...
class CaptionModel(nn.Module):
    ...
    #     print("触发了CaptionModel，请检查逻辑问题")
    #     mode = kwargs.get('mode', 'forward')
    #     if 'mode' in kwargs:
    #         del kwargs['mode']
    #     return getattr(self, '_' + mode)(*args, **kwargs)

    def beam_search(self, init_state, init_logprobs, prefix_kv, *args, **kwargs):
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    args_ = args[divm] + [state_table[divm]]
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.cuda(), *args_, prefix_kv)
                    # logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.cuda(), *(
                    #         args[divm] + [state_table[divm]]),self.prefix_kv)
