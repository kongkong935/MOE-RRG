...
def count_parameters_by_module(model, topn=30):
    module_param_counts = defaultdict(int)
    total_trainable = 0

    for name, param in model.named_parameters():
        if param.requires_grad:
            # 取名字前两层作为模块名（如 visual_extractor.model, encoder_decoder.model）
            module_prefix = ".".join(name.split(".")[:2])
            module_param_counts[module_prefix] += param.numel()
            total_trainable += param.numel()

    print("📊 各模块参数统计（只统计 requires_grad=True 的参数）：\n")
    for module_name, count in sorted(module_param_counts.items(), key=lambda x: x[1], reverse=True)[:topn]:
        print(f"{module_name:<30}  {count/1e6:>6.2f} M")

    print(f"\n🚀 总可训练参数数目：{total_trainable/1e6:.2f} M")
